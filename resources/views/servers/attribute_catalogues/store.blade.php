...
@section('content')
@php
$url = $config['method'] == 'create' ? route('attribute.catalogue.store') : route('attribute.catalogue.update',
    ...
    <div class="body d-flex py-3">
        ...
        <div class="container-xxl">
            ...
            <div class="row align-items-center">
                <div class="border-0 mb-4">
                    <div class="card-header py-3 no-bg bg-transparent d-flex align-items-center px-0 justify-content-between border-bottom flex-wrap">
                        ...
                        <button type="submit" class="btn btn-primary py-2 px-5 text-uppercase btn-set-task w-sm-100">{{__('messages.saveButton')}}</button>
                    </div>
                </div>
            </div> <!-- Row end  -->
            ...
            <div class="row g-3 mb-3 justify-content-center ">
                ...
                {{-- Aside --}}
                @include('servers.attribute_catalogues.blocks.aside')

                <div class="col-lg-8">
                    {{-- Main --}}
                    @include('servers.includes.content', ['model'=> $attributeCatalogue ?? []])

                    {{-- Album --}}
                    @include('servers.includes.album')

                    {{-- Seo --}}
                    @include('servers.includes.seo', ['model'=> $attributeCatalogue ?? []])
                </div>
            </div><!-- Row end  -->
